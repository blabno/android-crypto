...
public class AndroidCryptoCreateSigningKeyTest extends AbstractAndroidCryptoTest {

    @Test
    public void createSigningKey___access_level_is_always___creates_properly_configured_key() throws Exception {
        ...
        PublicKey key = crypto.createSigningKey(alias, accessLevel, invalidateOnNewBiometry);
        assertNotNull(key);
        ...
        assertEquals(KeyProperties.PURPOSE_SIGN + KeyProperties.PURPOSE_VERIFY, keyInfo.getPurposes());
    }

    @Test
    public void createSigningKey___access_level_is_unlocked_device___creates_properly_configured_key() throws Exception {
        String alias = faker.app().name();
    ...
    @Test
    public void createSigningKey___access_level_is_authentication_required___creates_properly_configured_key() throws Exception {
        String alias = faker.app().name();
    ...
    @Test
    public void createSigningKey___aliasAlreadyTaken() throws Exception {
        ...
        assertThrows(AliasConflictException.class, () -> crypto.createSigningKey(signingKeyAlias, AndroidCrypto.AccessLevel.ALWAYS, false));
    }

    @Test
    public void createRemoveSigningKey() throws Exception {
        createRemoveKeyScenario(alias -> {
            try {
                return crypto.createSigningKey(alias, AndroidCrypto.AccessLevel.ALWAYS, false);
            } catch (Exception e) {
